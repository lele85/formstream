...
var mergeStream = require("ordered-merge-stream");
...
FormStream.prototype.getStream = function () {
  ...
  this._buffers.forEach(function(buffer) {
    streams.push(streamifier.createReadStream(buffer[0]));
    streams.push(streamifier.createReadStream(buffer[1]));
    streams.push(streamifier.createReadStream(NEW_LINE_BUFFER));
  });

  this._streams.forEach(function(stream) {
    streams.push(streamifier.createReadStream(stream[0]));
    streams.push(stream[1]);
    streams.push(streamifier.createReadStream(NEW_LINE_BUFFER));
  });
  streams.push(streamifier.createReadStream(this._endData));
